...
@section('content')
    <section class="py-10 px-6 bg-white">
        <div class="p-6 space-y-6">
            <div>
                <h1 class="text-3xl font-[Poppins] text-[#1C6C73] mb-2">Fotos y notas del procedimiento</h1>
                    ...
                    @foreach ($folderNames as $key => $label)
                        ...
                        <div class="swiper-slide text-center {{ $bloquear ? 'opacity-50 pointer-events-none' : '' }}"
                            ...
                            <div class="flex flex-col items-center">
                                <a href="{{ route('panel.procedimientos.fotos', ['num_med_record' => $num_med_record, 'step' => $key]) }}"
            ...
            <form id="uploadForm" disabled
                action="{{ route('panel.procedimientos.fotos.upload', ['num_med_record' => $num_med_record, 'step' => $step]) }}"
                ...
                <input type="hidden" name="clinica" value="{{ $paciente->clinic }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        @foreach ($imagenes as $img)
                            <div class="border p-3 rounded-lg shadow hover:shadow-lg transition bg-white">
                                ...
                                <form action="{{ route('panel.procedimientos.fotos.delete') }}" method="POST"
        ...
        <div id="modalNotas"
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-6 relative animate-fadeIn">
                ...
                <form id="formNota" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <input type="hidden" name="procedure_type" value="procedure">
        ...
        <div class="mt-10 border-t pt-6">
            ...
            <div class="flex flex-col items-center space-y-4">
                <canvas id="firmaCanvas" width="400" height="200"
                    class="border border-gray-300 rounded-lg bg-white shadow-md"
        ...
        {{-- Script del Drag & Drop y sweetlaert --}}
        <script>
            function showUploadingAlert(total = null) {
            Swal.fire({
                title: 'Subiendo fotos…',
                html: total
                ? `Por favor espera.<br><b>Archivos:</b> ${total}`
                : 'Por favor espera…',
        ...
        <script>
            ...
            async function uploadFiles(files) {
                ...
                showUploadingAlert(files.length);

                const fd = new FormData(form);

                // limpia posibles archivos previos
                fd.delete('foto[]');
                ...
                try {
                    ...
                    icon: 'success',
                    title: 'Fotos subidas',
                    text: `${files.length} archivo(s) cargados correctamente`,
                    timer: 1500,
                    showConfirmButton: false
                    });

                    // refresca para ver la galería
                    ...
                    icon: 'error',
                    title: 'Error',
                    text: err.message || 'No se pudieron subir las fotos'
                    });
                }
                }
            ...
            ['dragenter','dragover'].forEach(evt => {
                ...
                e.preventDefault();
                form.classList.add('border-blue-400', 'bg-blue-50');
                });
            });
            ...
            ['dragleave','drop'].forEach(evt => {
                form.addEventListener(evt, (e) => {
                e.preventDefault();
                form.classList.remove('border-blue-400', 'bg-blue-50');
                });
            });

            form.addEventListener('drop', (e) => uploadFiles(e.dataTransfer.files));
            });
            </script>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // (opcional) habilitar botones cuando hay trazo
  const btnGuardar = document.getElementById('btnGuardarFirma');
  const btnEliminar = document.getElementById('btnEliminarFirma');

  function setButtons() {
    if (btnGuardar) btnGuardar.disabled = !hasStroke;
    // eliminar solo si existe firma guardada; si quieres lo dejamos como está
  }

  function getPos(evt) {
    const rect = canvas.getBoundingClientRect();
    // Escala por si el canvas se ve en CSS diferente al width/height real
  ...
  function startDraw(evt) {
    ...
    // captura el puntero para no perder el trazo si se sale del canvas
  ...
  function cargarFirma() {
    fetch("{{ route('panel.procedimientos.buscarFirma') }}", {
      ...
      body: JSON.stringify({ num_med, fase, clinic })
    })
    .then(res => res.json())
    .then(data => {
      ...
      if (data.success && data.url) {
        ...
        div.innerHTML = `<p class="text-gray-500">No hay firma registrada para esta fase.</p>`;
  ...
  document.getElementById('btnGuardarFirma')?.addEventListener('click', function () {
    ...
    fetch("{{ route('panel.procedimientos.guardarFirma') }}", {
      ...
      body: JSON.stringify({ firma: dataURL, num_med, fase, clinic })
    })
    .then(res => res.json())
    .then(data => {
      if (data.success) {
